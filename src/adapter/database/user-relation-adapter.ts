...
export class UserRelationAdapter implements UserRelationRepository {
    ...
    create(data: { sourceId: string; targetId: string; label?: string }): Promise<UserRelation> {
    ...
    findBySourceAndTarget(sourceId: string, targetId: string): Promise<UserRelation | null> {
        return this.prisma.userRelation.findFirst({ where: { sourceId, targetId, isDeleted: false } });
    }
    ...
    findBySourceId(sourceId: string, skip?: number, take?: number): Promise<UserRelationWithTarget[]> {
        return this.prisma.userRelation.findMany({
            where: { sourceId, isDeleted: false, target: { isDeleted: false } },
            include: { target: { select: { id: true, name: true, cpf: true } } },
            skip,
            take,
    ...
    findById(id: string): Promise<UserRelation | null> {
        return this.prisma.userRelation.findFirst({ where: { id, isDeleted: false } });
    }
