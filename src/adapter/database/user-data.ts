...
export class UserDataAdapter implements UserDataRepository {
    ...
    findById(id: string): Promise<UserDataModel | null> {
        return this.prisma.userData.findFirst({ where: { id, isDeleted: false } });
    }
    ...
    findByIdWithRelations(id: string): Promise<UserDataWithRelations | null> {
        return this.prisma.userData.findFirst({
            ...
            include: {
                relations: {
                    where: { isDeleted: false, target: { isDeleted: false } },
                    include: {
                        target: {
    ...
    findByCpf(cpf: string): Promise<UserDataModel | null> {
        return this.prisma.userData.findFirst({ where: { isDeleted: false, cpf } });
    }

    findByRg(rg: string): Promise<UserDataModel | null> {
        return this.prisma.userData.findFirst({ where: { isDeleted: false, rg } });
    }
...
data });
    ...
    async findAllPartners(): Promise<PartnerItem[]> {
        const rows = await this.prisma.userData.findMany({
            where: { isDeleted: false, isPartner: true },
            select: { id: true, name: true, avatar: true, partnerLogo: true, partnerUrl: true, cnpj: true },
            orderBy: [{ partnerOrder: { sort: 'asc', nulls: 'last' } }, { name: 'asc' }],
        });
        return rows.map((r: typeof rows[0]) => ({ id: r.id, name: r.name, avatarUrl: r.avatar, partnerLogoUrl: r.partnerLogo, partnerUrl: r.partnerUrl, cnpj: r.cnpj }));
    }

    async reorderPartners(ids: string[]): Promise<void> {
        await this.prisma.$transaction(
            ids.map((id, index) =>
                this.prisma.userData.update({ where: { id }, data: { partnerOrder: index } }),
            ),
        );
    }
}
