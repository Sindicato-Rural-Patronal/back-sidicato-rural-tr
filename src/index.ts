...
server.register(swagger, {
  openapi: {
    ...
    info: {
      ...
      description: 'API para gerenciamento de usuários, cursos e regras de permissão do Sindicato Rural',
      version: '1.0.0',
    },
    components: {
      securitySchemes: {
...
async function firstInitialize() {
  ...
  if (superRule) {
    const missingPerms = ALL_PERMITIONS.filter((p: string) => !(superRule!.permitions as string[]).includes(p));
